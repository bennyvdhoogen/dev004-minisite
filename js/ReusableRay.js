...
THREE.ReusableRay.prototype = {
  ...
  intersectMesh: function (object, parent) {
    ...
    distance = this.distanceFromIntersection(this.origin, this.direction, object.matrixWorld.getPosition());
		var scale = new THREE.Frustum();
		scale.setFromMatrix(object.matrixWorld);
//var scale = THREE.Frustum.set(object.matrixWorld.getColumnX().length(), object.matrixWorld.getColumnY().length(), object.matrixWorld.getColumnZ().length());
    ...
    object.matrixWorld.extractRotation(object.matrixWorld);
    ...
    for (f = 0, fl = geometry.faces.length; f < fl; f++) {
      ...
      vector = objMatrix.multiplyVector3(vector.copy(face.centroid)).sub(originCopy);
      normal = object.matrixWorld.multiplyVector3(normal.copy(face.normal));
